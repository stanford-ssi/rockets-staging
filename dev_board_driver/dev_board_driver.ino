...
void driveMotor() {
  ...
  if (counter % 50000 == 0) {
    bool loada = tmc26XStepper.isOpenLoadA();
    if (loada == true) {
      Serial.println("Load A: connected");
    } else if (loada == false) {
      Serial.println("Load A: not connected");
    }
    bool loadb = tmc26XStepper.isOpenLoadB();
    if (loadb == true) {
      Serial.println("Load B: connected");
    } else if (loadb == false) {
      Serial.println("Load B: not connected");
    }
  }
}
/*
